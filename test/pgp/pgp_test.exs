defmodule Bonfire.Mailer.PGPTest do
  ...
  describe "prepare_deliveries/1 with pgp disabled" do
    test "returns original email unchanged, even with a key available" do
      Req.Test.stub(:pgp_key_lookup, fn conn ->
      ...
      email = base_email()
      assert Bonfire.Mailer.prepare_deliveries(email) == [email]
    end
  end

  if Bonfire.Common.Extend.module_enabled?(Bonfire.Mailer.PGP) do
